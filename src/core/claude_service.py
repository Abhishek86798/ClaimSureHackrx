...
class ClaudeStatus(Enum):
    """Status of Claude 3.5 Sonnet service"""
    AVAILABLE = "available"
...
class ClaudeService:
    """
    Manages interactions with the Anthropic Claude 3.5 Sonnet API.
    Handles API key loading, rate limiting, retries, and intelligent fallbacks.
    """
    
    def __init__(self):
        """Initialize Claude service with API key and configuration"""
        self.api_key = os.getenv('ANTHROPIC_API_KEY')
        self.client = None
        self.status = ClaudeStatus.UNAVAILABLE
        ...
        if self.api_key:
            try:
                ...
                self.status = ClaudeStatus.ERROR
        else:
            logging.warning("No Anthropic API key found. Claude service unavailable.")
    ...
    def _test_connection(self):
        ...
        try:
            ...
            self.status = ClaudeStatus.AVAILABLE
            logging.info("Claude 3.5 Sonnet service initialized successfully")
        except Exception as e:
            logging.error(f"Claude connection test failed: {e}")
            self.status = ClaudeStatus.ERROR
    
    def get_status(self) -> ClaudeStatus:
    ...
    def process_insurance_query(self, 
                              ...
                              claim_details: Optional[Dict] = None) -> Tuple[str, float, ClaudeStatus]:
        """
        Process an insurance-related query using Claude 3.5 Sonnet
        ...
        try:
            ...
            self.status = ClaudeStatus.RATE_LIMITED
            logging.warning("Claude API rate limit exceeded")
            return "", 0.0, self.status
        except anthropic.APIError as e:
            if "quota" in str(e).lower():
                self.status = ClaudeStatus.QUOTA_EXCEEDED
            else:
                self.status = ClaudeStatus.ERROR
            ...
            self.status = ClaudeStatus.ERROR
            logging.error(f"Unexpected error in Claude service: {e}")
            return "", 0.0, self.status
    
    def batch_process_queries(self, 
                            queries: List[str], 
                            contexts: Optional[List[str]] = None) -> List[Tuple[str, float, ClaudeStatus]]:
        """
